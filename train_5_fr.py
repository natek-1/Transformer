...
class LayerNormalization(nn.Module):
    ...
    def __init__(self, features: int, eps: float = 10**-6) -> None: # We define epsilon as 0.000001 to avoid division by zero
        super().__init__()
        self.eps = eps
        
        # We define alpha as a trainable parameter and initialize it with ones
        self.alpha = nn.Parameter(torch.ones(features)) # One-dimensional tensor that will be used to scale the input data
        
        # We define bias as a trainable parameter and initialize it with zeros
        self.bias = nn.Parameter(torch.zeros(features)) # One-dimensional tenso that will be added to the input data
        
    def forward(self, x):
        mean = x.mean(dim = -1, keepdim = True) # Computing the mean of the input data. Keeping the number of dimensions unchanged
        var = x.var(dim = -1, keepdim = True) # Computing the standard deviation of the input data. Keeping the number of dimensions unchanged
...
class ResidualConnection(nn.Module):
    def __init__(self, feature: int,dropout: float) -> None:
        ...
        self.norm = LayerNormalization(feature) # We use a normalization layer 
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, features: int,self_attention_block: MultiHeadAttentionBlock, feed_forward_block: FeedForwardBlock, dropout: float) -> None:
        ...
        self.residual_connections = nn.ModuleList([ResidualConnection(features, dropout) for _ in range(2)]) # 2 Residual Connections with dropout
...
class Encoder(nn.Module):
    ...
    def __init__(self, feature: int,layers: nn.ModuleList) -> None:
        ...
        self.norm = LayerNormalization(feature) # Layer for the normalization of the output of the encoder layers
...
class Decoder(nn.Module):
    ...
    def __init__(self, feature: int, layers: nn.ModuleList) -> None:
        ...
        self.layers = layers
        self.norm = LayerNormalization(feature) # Layer to normalize the output
...
def get_config():
    return{
        ...
        'lang_src': 'en',
        'lang_tgt': 'fr',
        'model_folder': 'weightsv3_en_fr_feature_layenorm_no_bais_nologsoftmax',
